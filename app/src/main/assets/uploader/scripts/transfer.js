$(function () {
    ...
    function fillFilesContainer() {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            row.append('<div class="column download" style="visibility:hidden" title="' + STRINGS.DOWNLOAD_FILE + '" download="' + fileInfo.name + '"></div>');
        ...
        $.post("/send_file_name", {"filename": fileName}, function () {
            $.ajaxFileUpload({
                ...
                success: function (data, status) {
                    ...
                    $('<div class="column download" style="visibility:hidden" title="' + STRINGS.DOWNLOAD_FILE + '"></div>')
                    //.click(downloadBook)
                        .appendTo(row);
