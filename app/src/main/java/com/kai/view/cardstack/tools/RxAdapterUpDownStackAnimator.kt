 ...
 */
class RxAdapterUpDownStackAnimator(rxCardStackView: RxCardStackView?) : RxAdapterAnimator(
    rxCardStackView!!
) {
    override fun itemExpandAnimatorSet(viewHolder: RxCardStackView.ViewHolder?, position: Int) {
        ...
        val oa =
            ObjectAnimator.ofFloat(itemView, View.Y, itemView.y, mRxCardStackView.getChildAt(0).y)
        mSet!!.play(oa)
        ...
        for (i in 0 until mRxCardStackView.childCount) {
            ...
            if (i > mRxCardStackView.selectPosition && collapseShowItemCount < mRxCardStackView.numBottomShow) {
                ...
                val oAnim =
                    ObjectAnimator.ofFloat(child, View.Y, child.y, mRxCardStackView.getChildAt(0).y)
                mSet!!.play(oAnim)
            } else {
                val oAnim = ObjectAnimator.ofFloat(
                    ...
                    View.Y,
                    child.y,
                    mRxCardStackView.showHeight.toFloat()
                )
                mSet!!.play(oAnim)
            }
        }
    ...
    override fun itemCollapseAnimatorSet(viewHolder: RxCardStackView.ViewHolder?) {
        ...
        for (i in 0 until mRxCardStackView.childCount) {
            ...
            val oAnim: ObjectAnimator = ObjectAnimator.ofFloat<View>(
                child, View.Y, child.y,
                if (childTop - mRxCardStackView.rxScrollDelegate!!.viewScrollY < mRxCardStackView.getChildAt(
                        0
                    ).y
                ) mRxCardStackView.getChildAt(0).y else (childTop - mRxCardStackView.rxScrollDelegate!!.viewScrollY).toFloat()
            )
            mSet!!.play(oAnim)
