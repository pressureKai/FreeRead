package com.kai.view.cardstack
...
import com.kai.view.cardstack.tools.*
import java.util.*
...
class RxCardStackView : ViewGroup, RxScrollDelegate {
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int) {
        val array = context.obtainStyledAttributes(
            attrs,
            R.styleable.RxCardStackView,
            defStyleAttr,
            defStyleRes
        )
        overlapGaps =
            array.getDimensionPixelSize(R.styleable.RxCardStackView_stackOverlapGaps, dp2px(20))
        overlapGapsCollapse = array.getDimensionPixelSize(
            R.styleable.RxCardStackView_stackOverlapGapsCollapse,
            dp2px(20)
        )
        duration = array.getInt(
            R.styleable.RxCardStackView_stackDuration,
            RxAdapterAnimator.ANIMATION_DURATION
        )
        setAnimationType(
            array.getInt(
                R.styleable.RxCardStackView_stackAnimationType,
                UP_DOWN_STACK
            )
        )
    ...
    fun setAnimationType(type: Int) {
        val rxAdapterAnimator: RxAdapterAnimator
        rxAdapterAnimator = when (type) {
    ...
    fun getViewHolder(i: Int): ViewHolder? {
        ...
        if (mViewHolders!!.size <= i || mViewHolders!![i].mItemViewType != mRxAdapterStack!!.getItemViewType(
                i
            )
        ) {
                    ...
                    if (childCount > 0) {
                        if (Math.abs(initialVelocity) > mMinimumVelocity) {
                            ...
                            if (mScroller!!.springBack(
                                    viewScrollX, rxScrollDelegate!!.viewScrollY, 0, 0, 0,
                                    scrollRange
                                )
                            ) {
                                postInvalidate()
    ...
    class LayoutParams : MarginLayoutParams {
        ...
        constructor(c: Context, attrs: AttributeSet?) : super(c, attrs) {
            val array = c.obtainStyledAttributes(attrs, R.styleable.RxCardStackView)
            mHeaderHeight =
                array.getDimensionPixelSize(R.styleable.RxCardStackView_stackHeaderHeight, -1)
        }
